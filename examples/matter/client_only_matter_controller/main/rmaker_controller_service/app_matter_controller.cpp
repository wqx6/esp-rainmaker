                                        ...
                                        esp_rmaker_write_ctx_t *ctx)
{
    ...
    if (rmaker_group_id) {
        size_t size_to_copy = strlen(rmaker_group_id);
...
static esp_err_t update_access_token(const char *access_token)
{
    ...
    if (access_token) {
        size_t size_to_copy = strlen(access_token);
                                   ...
                                   esp_rmaker_write_ctx_t *ctx)
{
    ...
    if (user_token) {
        size_t size_to_copy = strlen(user_token);
                                 ...
                                 esp_rmaker_write_ctx_t *ctx)
{
    ...
    if (base_url) {
        size_t size_to_copy = strlen(base_url);
...
esp_err_t matter_controller_handle_update()
{
    ...
    if (handle->base_url && handle->user_token && !handle->access_token) {
        ...
        ESP_RETURN_ON_ERROR(s_matter_controller_callback(handle, MATTER_CONTROLLER_CALLBACK_TYPE_AUTHORIZE),
                            TAG, "Failed on authorizing");
