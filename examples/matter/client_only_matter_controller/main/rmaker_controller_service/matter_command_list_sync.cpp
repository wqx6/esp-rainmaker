...
extern "C" esp_err_t matter_command_list_sync_for_device_list(const char *endpoint_url,
                                                              const char *access_token,
                                                              matter_device_t *dev_list)
{
    if (!endpoint_url || !access_token) {
    ...
    for (matter_device_t *dev = dev_list; dev; dev = dev->next) {
        ...
        err = update_rainmaker_node_metadata(endpoint_url, access_token, dev->rainmaker_node_id, body.c_str());
        if (err != ESP_OK) {
